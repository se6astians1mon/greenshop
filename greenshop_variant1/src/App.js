  ...
  const signInAn = () => {
    ...
    if (userID == null) {
      ...
      userSignInAnonymously().then((user) => {
       userExists(mailID, props.userID).then((result) => {
      ...
      <Router history={history}>
        ...
        <Route path='/instructions'
          render={() => {
            ...
            signInAn(userID)
            return (<Instructions next={() => { history.push("/store", {from: history.location}); }} />)
          }}
        />
        <Route path='/store'
          render={() => {
		...
		const redirectlink = "https://www.merciderepondre.com/WebProd/cgi-bin/askiaext.dll?Action=StartSurvey&SurveyName=RET2385&Broker=99&BrokerPanelId="+mailID+"&ID_EXP="+1; 
